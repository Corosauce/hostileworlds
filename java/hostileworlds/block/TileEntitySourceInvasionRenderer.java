...
public class TileEntitySourceInvasionRenderer extends TileEntitySpecialRenderer
{
    ...
    public void renderEndPortalTileEntity(TileEntitySourceInvasion par1TileEntityEndPortal, double par2, double par4, double par6, float par8)
    {
        ...
        float var9 = (float)RenderManager.instance.viewerPosX;
        float var10 = (float)RenderManager.instance.viewerPosY;
        float var11 = (float)RenderManager.instance.viewerPosZ;
        ...
        for (int var14 = 0; var14 < 16; ++var14)
        {
            ...
            float time2 = (float) Math.sin(par1TileEntityEndPortal.getWorldObj().getWorldTime() / 200F);
            float time3 = (float) Math.sin(par1TileEntityEndPortal.getWorldObj().getWorldTime() / 40F);
            float time4 = (float) Math.cos(par1TileEntityEndPortal.getWorldObj().getWorldTime() / 40F);
            ...
            //if (!generateTexture) {
	            ...
	            GL11.glTexGen(GL11.GL_Q, GL11.GL_EYE_PLANE, this.func_76907_a((float)Math.cos((float)par1TileEntityEndPortal.getWorldObj().getWorldTime() / 40F) * 0.2F, 0.7F, 0.0F, 0.0F));
            //}
            ...
            GL11.glTranslatef((float) (var14 / 1.5F * Math.cos(par1TileEntityEndPortal.getWorldObj().getWorldTime() / 50F)), 0.0F, 0.0F);
            GL11.glTranslatef(0.0F, (float) (var14 / 1.5F * Math.sin(par1TileEntityEndPortal.getWorldObj().getWorldTime() / 50F)), 0.0F);
            ...
            //temp debug
            //type = 2;
            rotation = par1TileEntityEndPortal.getBlockMetadata();
            ...
            if (type == 0) {
            	...
            	float shift = (float)Math.cos((float)par1TileEntityEndPortal.getWorldObj().getWorldTime() / 50F);
            	float shift2 = (float)Math.cos((float)par1TileEntityEndPortal.getWorldObj().getWorldTime() / 100F);
            	float shift3 = (float)Math.cos((float)par1TileEntityEndPortal.getWorldObj().getWorldTime() / 60F);
            ...
            float fade = (float)Math.cos((float)par1TileEntityEndPortal.getWorldObj().getWorldTime() / 20F) * 0.02F;
            ...
            double extraRadius = 1D;
            
            //GL11.glScalef(0.5F, 0.5F, 0.5F);
            var24.addVertex(par2 - extraRadius, par4 + (double)var13 + 1D, par6 - extraRadius);
            var24.addVertex(par2 - extraRadius, par4 + (double)var13 + 1D, par6 + 1.0D + extraRadius);
            var24.addVertex(par2 + 1.0D + extraRadius, par4 + (double)var13 + 1D, par6 + 1.0D + extraRadius);
            var24.addVertex(par2 + 1.0D + extraRadius, par4 + (double)var13 + 1D, par6 - extraRadius);
            var24.draw();
            GL11.glPopMatrix();
