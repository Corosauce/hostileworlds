...
public class Zombie extends EntityInvader implements IAdvPF {
	...
	public void onLivingUpdate() {
		...
		if (!worldObj.isRemote) {
			List<Entity> var2 = this.worldObj.getEntitiesWithinAABB(EntityFallingBlock.class, this.boundingBox.expand(1.0D, 3D, 1.0D));
		...
		if (!worldObj.isRemote && stackMode) {
			...
			if (var2.size() > 1) {
				for (int i = 0; i < var2.size(); i++) {
					if (var2.get(i) != this && var2.get(i) instanceof Zombie && this.getEntityId() > var2.get(i).getEntityId() && this.boundingBox.minY+1.5D > var2.get(i).boundingBox.minY && this.boundingBox.minY < var2.get(i).boundingBox.maxY/*this.boundingBox.minY-0.1D < var2.get(i).boundingBox.maxY && this.boundingBox.minY < var2.get(i).boundingBox.maxY*/) {
			...
			if (ent != null) {
				...
				if (tryHoist > 0) {
					...
					if (!worldObj.isRemote) {
						...
						Block id = worldObj.getBlock((int)(tryX), (int)(tryY), (int)(tryZ));
						if (!CoroUtilBlock.isAir(id) && (id.getMaterial() == Material.leaves || id.getMaterial() == Material.plants)) {
							//System.out.println("remove leafs!");
							worldObj.setBlock((int)(tryX), (int)(tryY), (int)(tryZ), Blocks.air);
						}
					}
