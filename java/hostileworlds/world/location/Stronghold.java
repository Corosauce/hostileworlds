...
public class Stronghold extends TownObject implements ICustomGen {
	...
	public void tickUpdate() {
		...
		if (getWorld().getTotalWorldTime() % 20 == 0) {
			System.out.println("stronghold tick");
	...
	public void spawnEntities() {
		...
		spawnEntityRel("archer", Vec3.createVectorHelper(spawnRadius, spawn.posY + wallHeight + 1, spawnRadius - fix));
		spawnEntityRel("archer", Vec3.createVectorHelper(spawnRadius, spawn.posY + wallHeight + 1, -spawnRadius));
		spawnEntityRel("archer", Vec3.createVectorHelper(-spawnRadius, spawn.posY + wallHeight + 1, spawnRadius - fix));
		spawnEntityRel("archer", Vec3.createVectorHelper(-spawnRadius, spawn.posY + wallHeight + 1, -spawnRadius));
		
		spawnEntityRel("guard", Vec3.createVectorHelper(-13, spawn.posY + 1, -2));
		spawnEntityRel("guard", Vec3.createVectorHelper(-13, spawn.posY + 1, 2));
		
		spawnEntityRel("guard", Vec3.createVectorHelper(13, spawn.posY + 1, -2));
		spawnEntityRel("guard", Vec3.createVectorHelper(13, spawn.posY + 1, 2));
		
		spawnEntityRel("guard", Vec3.createVectorHelper(-2, spawn.posY + 1, -13));
		spawnEntityRel("guard", Vec3.createVectorHelper(+2, spawn.posY + 1, -13));
		
		spawnEntityRel("guard", Vec3.createVectorHelper(-2, spawn.posY + 1, 13 - fix));
		spawnEntityRel("guard", Vec3.createVectorHelper(+2, spawn.posY + 1, 13 - fix));
	}
