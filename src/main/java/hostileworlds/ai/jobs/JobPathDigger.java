...
public class JobPathDigger extends JobGroupHorde {
	...
	public void tick() {
		...
		//weird temp kill
		if (ent.worldObj.getBlock((int)ent.posX, (int)ent.posY-1, (int)ent.posZ) != HostileWorlds.blockBloodyCobblestone) {
			//dbg("weawewaeawe");
			//ent.setDead();
			//return;
		}
	...
	public boolean trySetNextDigBlock(int offset, boolean usePosition, int afterTraceOffset) {
		...
		if (coords != null) {
			...
			if (!CoroUtilBlock.isAir(id) && (id.getMaterial() != Material.water)) {
				if (ent.worldObj.getTileEntity(coords.posX, coords.posY, coords.posZ) == null && !HostileWorlds.blocksList.contains(ent.worldObj.getBlock(coords.posX, coords.posY, coords.posZ))) {
