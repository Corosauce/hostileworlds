...
public class EntityWormFire extends EntityWorm {
	...
	protected void applyEntityAttributes() {
		...
		tempMaxHealth = 100; //because code is retarded and its not set yet for some reason
        this.getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(tempMaxHealth);
	}
	...
	public void spawnBlock(WormNode node) {
		MovingBlock mb = new MovingBlock(worldObj, Blocks.mossy_cobblestone, 0);
	...
	public void setDead()
    {
        ...
        /*if (worldObj.isRemote) {
    		for (int i = 0; i < 60; i++) {
	    		...
	    		EntityReddustFX particle = new EntityReddustFX(worldObj, this.posX + 0.5F + rand.nextDouble()*range - rand.nextDouble()*range, this.posY + rand.nextDouble()*range, this.posZ + 0.5F + rand.nextDouble()*range - rand.nextDouble()*range, 2F, rand.nextFloat(), 0F, 0F);
	    		particle.motionX = rand.nextDouble()*range - rand.nextDouble()*range;
	    		particle.motionY = rand.nextDouble()*range - rand.nextDouble()*range;
	    		particle.motionZ = rand.nextDouble()*range - rand.nextDouble()*range;
	...
	public void onUpdate() {
		...
		if (!worldObj.isRemote) {
	    	...
	    	for (int xx = (int)posX-range; xx < posX+range; xx++) {
	    		for (int yy = (int)posY-range; yy < posY+range; yy++) {
	    			for (int zz = (int)posZ-range; zz < posZ+range; zz++) {
	    				...
	    				if (!CoroUtilBlock.isAir(id) && id != Blocks.fire && worldObj.getTileEntity(xx, yy, zz) == null && id.getMaterial() != Material.water && !HostileWorlds.blocksList.contains(id)) {
	    					if (!worldObj.isRemote) {
	    						if (this.getDistance(xx, yy, zz) <= range) {
		    						//System.out.println("explode block!");
	    							//cant eat catacombs and ground
	    							if (id.getBlockHardness(worldObj, xx, yy, zz) != -1 && (yy > 7 || posY < 7)) {
	    							...
	    							if (CoroUtilBlock.isAir(worldObj.getBlock(xx, yy+1, zz)) && worldObj.rand.nextInt(10) == 0) {
	    								worldObj.setBlock(xx, yy+1, zz, Blocks.fire);
	    							}
	    						}
	    					}
	    				}
	    	...
	    	float targetRange = (float) agent.maxDistanceFromHome;
	    	moveSpeedMax = 1.5F;
	    	
	    	Vec3 targetCoord = Vec3.createVectorHelper(agent.homeX, agent.homeY, agent.homeZ);
	    	boolean chasingAPlayer = false;
			
			//temp ai testing
			EntityPlayer player = worldObj.getClosestPlayerToEntity(this, targetRange);
			if (player != null && this.getDistance(targetCoord.xCoord, targetCoord.yCoord, targetCoord.zCoord) < agent.maxDistanceFromHome) {
				chasingAPlayer = true;
				targetCoord.xCoord = player.posX;
				targetCoord.yCoord = player.posY;
				targetCoord.zCoord = player.posZ;
			}
			...
			//keep in this file
			double vecX = targetCoord.xCoord - posX;
			double vecZ = targetCoord.zCoord - posZ;
			...
			smoothPitch = (float)(Math.atan2(targetCoord.yCoord - posY, Math.sqrt(vecX * vecX + vecZ * vecZ)) * 180.0D / Math.PI);
			
			this.setRotation(smoothYaw + 90, rotationPitch);
			
			//System.out.println(180 - Math.abs(aimYaw));
			//if heading towards target accurately enough?
			if (180 - Math.abs(aimYaw) < 30F) {
				if (speed < moveSpeedMax) {
					moveTowards(this, targetCoord, moveSpeed);
					//motionY *= 1.01F;
				} else {
					motionX *= 0.99F;
					motionY *= 0.99F;
				...
				//speed away once past
				if (this.getDistance(targetCoord.xCoord, targetCoord.yCoord, targetCoord.zCoord) < 90F) {
			...
			float distt = (float) this.getDistance(targetCoord.xCoord, targetCoord.yCoord, targetCoord.zCoord);
			if (distt < 8F*2F) {
	...
	public void moveTowards(Entity ent, Vec3 targ, float speed) {
		double vecX = targ.xCoord - ent.posX;
		double vecY = targ.yCoord - ent.posY;
		double vecZ = targ.zCoord - ent.posZ;
