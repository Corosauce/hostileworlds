...
public class AreaConverter {
	...
	public int convertBlock = ModConfigFields.areaConverterReplaceBlocksWith;
				...
				if (Math.sqrt(sourceCoord.getDistanceSquared(tryX, tryY, tryZ)) < curRange) {
					...
					if (id == 0) {
						...
						if (Math.abs(topY - sourceCoord.posY) < convertRangeMax) {
							...
							id = world.getBlockId(tryX, tryY, tryZ);
						}
					}
					if (id != 0 && id != convertBlock && id != HostileWorlds.blockBloodyCobblestone.blockID && 
							Block.blocksList[id].blockMaterial.isSolid() && 
							world.getBlockTileEntity(tryX, tryY, tryZ) == null) {
